...
void scenes_internal_goto(uint32_t index) {
  ...
  if (current) {
    {
      ...
      rectify_map_set(map, "system", sizeof(char) * (strnlen(current->name, 128) + 1), current->name);
  ...
  {
    RectifyMap *map = rectify_map_create();
    rectify_map_set(map, "system", sizeof(char) * (strnlen(target->name, 128) + 1), target->name);
    gossip_post("systems", MSG_SYSTEM_START, map);
  }
  {
    RectifyMap *map = rectify_map_create();
    rectify_map_set(map, "scene", sizeof(char) * (strnlen(target->name, 128) + 1), target->name);
